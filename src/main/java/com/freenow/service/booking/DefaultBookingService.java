...
public class DefaultBookingService implements BookingService{
    ...
    public CarDO unBookCar(DriverDO currentDriver,CarDO selectedCar) throws CarAlreadyInUseException {
        if(currentDriver.getCarId().equals(selectedCar.getId())) {
        ...
        if(selectedCar.getIsAvaliable())  throw new CarAlreadyInUseException("Car "+selectedCar.getId()+" cannot be suspended, It is available to book ");
          throw new CarAlreadyInUseException("Car "+selectedCar.getId()+" cannot be suspended booked by other driver ");
